function CheckSite() {
    ...
    Write-Output "Checking to see if $SiteURL exists in the list..."
    ...
    if ($siteListItem.Count -eq 0) {
        Write-Output "Adding $siteURL to the list"
    ...
    $listSiteMetaData = "Lists/SiteMetadata"
    Write-Output "Check if ""$($listSiteMetaData)"" list exists in $($siteURL) site. Updating..."
    ...
    if (-not $listExists) {
        ...
        while (($retries -lt 20) -and ($pnpTemplateApplied -eq $false)) {
            try {
                ...
                $applyTemplate = Invoke-PnPSiteTemplate -Path $siteMetadataPnPTemplate -Connection $connSite -ErrorAction Stop
                $removeContentType = Remove-PnPContentTypeFromList -List "Site Metadata" -ContentType "Item" -Connection $connSite
        ...
        Write-Output "Updating Site Metadata List."
        $setValue = Get-PnPListItem -List "Site Metadata" -Connection $connSite | Set-PnPListItem -List "Site Metadata" -Identity { $_.Id } -Values $newListItemValues -Connection $connSite
    }
...
$SitesListSiteURL = $SiteCollectionFullURL
Write-Output "Connecting to: $($SitesListSiteURL)"
Write-Verbose "Connecting to: $($SiteCollectionFullURL)"
$connLanding = Helper-Connect-PnPOnline -Url $SitesListSiteURL
...
$pnpTemplates | ForEach-Object {
    $File = Get-PnPFile -Url "$($SiteCollectionRelativeURL)/pnptemplates/$($_.Name)" -Path $pnpTemplatePath -Filename $_.Name -AsFile -Force -Connection $connLanding
}
...
$siteCollectionListItems | ForEach {
    ...
    if ($siteExists) {
        ...
        if (($listbreadcrumbHTML -notlike "*$($updatedBreadcrumbHTML)*") -or ($listSiteTitle -ne $Site.Title) -or ($listSubsite -ne $updatedSubsite)) {
            ...
            Write-Output "$($Site.Title) exists in $($SiteListName) list. Updating..."
            $connLanding = Helper-Connect-PnPOnline -Url $SitesListSiteURL
            ...
            Write-Output "$($listSiteTitle) exists in $($SiteListName) list. No updates required."
        }
    }
    else {
